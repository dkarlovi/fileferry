...
const (
	...
	// Deduplicated means the destination already held an identical file, so
	// nothing was copied. For a move the source is still deleted; for a copy it
	// is simply left alone. This is not the happy path for a move (it means a
	// duplicate was downloaded), but it is not an error either.
	Deduplicated
)
...
// as-is and the source is deleted (for a move) or kept (for a copy), as if this
// run had performed the transfer, and Deduplicated is returned. If they differ,
// an error is returned and neither file is touched.
func TransferEntry(entry Entry, destPath string, op ffcfg.Operation) (TransferOutcome, error) {
	destDir := filepath.Dir(destPath)
	if err := os.MkdirAll(destDir, 0755); err != nil {
		return Transferred, fmt.Errorf("create dir %s: %w", destDir, err)
	}
	...
	if info, err := os.Stat(destPath); err == nil {
		if info.IsDir() {
			return Transferred, fmt.Errorf("destination %s is a directory", destPath)
		}
		return reconcileExisting(entry, destPath, op)
	} else if !os.IsNotExist(err) {
		return Transferred, fmt.Errorf("stat destination %s: %w", destPath, err)
	}
	...
	if size := entry.Size(); size >= 0 && written != size {
		os.Remove(tmpPath)
		return Transferred, fmt.Errorf("copy size mismatch for %s: wrote %d bytes, source reports %d", entry.DisplayPath(), written, size)
	}
	srcHash, err := hashEntry(entry)
	if err != nil {
		os.Remove(tmpPath)
		return Transferred, fmt.Errorf("re-read source %s for verification: %w", entry.DisplayPath(), err)
	}
	if srcHash != destHash {
		os.Remove(tmpPath)
		return Transferred, fmt.Errorf("verification failed for %s: source and copied file differ (SHA-256 %s != %s)", entry.DisplayPath(), srcHash, destHash)
	}

	if err := os.Rename(tmpPath, destPath); err != nil {
		os.Remove(tmpPath)
		return Transferred, fmt.Errorf("finalize %s: %w", destPath, err)
	}
	...
	if err := entry.Delete(); err != nil {
		return Transferred, fmt.Errorf("copied and verified to %s but failed to delete source %s: %w", destPath, entry.DisplayPath(), err)
	}
	return Transferred, nil
}
...
// a duplicate that was effectively already transferred here, so it is deleted
// (move) or left in place (copy); on a mismatch both files are left untouched
// and an error is returned.
func reconcileExisting(entry Entry, destPath string, op ffcfg.Operation) (TransferOutcome, error) {
	if _, err := compareDestination(entry, destPath); err != nil {
		return Transferred, err
	}

	// Identical content: this is a duplicate of a file already in place.
	if op == ffcfg.OperationCopy {
		return Deduplicated, nil
	}
	if err := entry.Delete(); err != nil {
		return Transferred, fmt.Errorf("source %s is a duplicate of %s but failed to delete: %w", entry.DisplayPath(), destPath, err)
	}
	return Deduplicated, nil
}

// PreviewTransfer reports what TransferEntry would do, without modifying either
// file. It is the dry-run counterpart of TransferEntry: if the destination
// already exists it hashes both files, returning Deduplicated when they match or
// an error when they differ; otherwise it returns Transferred. The outcome does
// not depend on the operation — only what TransferEntry then does to the source
// does — so it takes no operation argument.
func PreviewTransfer(entry Entry, destPath string) (TransferOutcome, error) {
...
// transfer (reconcileExisting) and dry-run (PreviewTransfer) paths.
func compareDestination(entry Entry, destPath string) (TransferOutcome, error) {
	destHash, err := hashFile(destPath)
	if err != nil {
		return Transferred, fmt.Errorf("hash existing destination %s: %w", destPath, err)
	}
	srcHash, err := hashEntry(entry)
	if err != nil {
		return Transferred, fmt.Errorf("re-read source %s for duplicate check: %w", entry.DisplayPath(), err)
	}
	if srcHash != destHash {
		return Transferred, fmt.Errorf("destination %s already exists and differs from source %s (SHA-256 %s != %s); leaving both untouched", destPath, entry.DisplayPath(), destHash, srcHash)
	}
	return Deduplicated, nil
}
